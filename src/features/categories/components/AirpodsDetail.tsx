...
import { SmoothScrollContainer } from "./smooth";
import { AirPodsTopNav } from "./Nav";
import { menuAirPodsInfo } from "@/features/categories/constants/AirPodsInfo";
...
const AirPodsDetail = async ({ params }: { params: { id: string } }) => {
  const airpodsCategory = await prisma.category.findUnique({
    where: { name: "AirPods" },
    select: { id: true },
  });

  if (airpodsCategory?.id !== params.id) {
    return <></>;
  }
  return (
